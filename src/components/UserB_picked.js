...
class UserB_picked extends Component {
    constructor(props) {
      ...
      // 讀取訂單列表
      fetch('http://localhost/foodstory_end/PHP-and-SQL-master/php/seller_orders/seller_orders_pay_API.php', {
            method: 'GET',
            mode:'cors',
            credentials: 'include',
            body: JSON.stringify(),
                      ...
                      <div className="row">
                        <div className="col">
                            <table class="table table-hover notoSans">
                                ...
                                <tbody>
                                {this.state.picked.map(picked=>
                                    <tr key={picked.Numb_sid} className="pointer">
                                    <th scope="row">{picked.Numb_sid}</th>
                                    <td>{picked.order_time}</td>
                                    <td>{picked.user_name}</td>
                                    <td>{picked.user_phone}</td>
                                    <td>{picked.total}</td>
                                    <td>已付款</td> 
                                    <td><span data-sid={picked.Numb_sid} data-toggle="modal" data-target="#check" className="btn_solid4 text-center text_decoration check">查看</span></td>                                  
                                    </tr>
                                )}
